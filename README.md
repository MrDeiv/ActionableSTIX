# Creating Actionable Rich Threat Intelligence Reports Merging Structured and Narrative Information Through Large Language Models

## Abstract
Effective cyber threat intelligence relies on data collection and the ability to contextualize and act upon that data swiftly. While STIX (Structured Threat Information Expression) provides a standardized format for sharing threat indicators, it often lacks the actionability analysts require. This work presents a novel pipeline powered by Large Language Models (LLMs) that, beginning with a STIX file and additional documents related to a specific cyber threat, produces a graph-based representation of the cyber attack. This representation is designed to capture the temporal evolution of the attack. Each graph node corresponds to a specific attacker action and is associated with a detailed natural-language description of that action, including its Pre-conditions, Post-conditions, and Indicators of Compromise. Consequently, the pipeline presents the threat with a higher detail level than the original STIX file. The resulting system effectively bridges the gap between raw threat data and informed, rapid decision-making.
