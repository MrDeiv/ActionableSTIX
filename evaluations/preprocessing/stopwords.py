...
with open("results_with_stopwords.json", "w") as f:
    json.dump(results, f)
...
for example in ds:
    question = example['question']
    context = " ".join([word for word in example['contexts'].split() if word not in stop_words])
    answer = agent(question=question, context=context)['answer']
    answers.append(answer)
    ground_truths.append(example['ground_truths'])
...
with open("results_without_stopwords.json", "w") as f:
    json.dump(results, f)